...
// if it present in the current page
if (!function_exists('trx_addons_check_revslider_in_content')) {
    ...
    function trx_addons_check_revslider_in_content($load) {
        // Check in Gutenberg
        if ( ! $load ) {
            $content = get_the_content();
            if ( function_exists('trx_addons_exists_gutenberg') && trx_addons_exists_gutenberg() ) {
            ...
            // Check in VC
            if ( ! $load && function_exists('trx_addons_exists_vc') && trx_addons_exists_visual_composer() ) {
                $load = is_string($content)
                    && strpos($content, '[trx_widget_slider') !== false     // Our widget slider with param "engine" == "revo"
                    && strpos($content, 'engine="revo"') !== false;
            }
            // Check in Elementor
            if ( ! $load && function_exists('trx_addons_exists_elementor') && trx_addons_exists_elementor() ) {
                ...
                if ( $cur_page_built_with_elementor ) {
                    ...
                    $load = is_string($meta)
                        && (
                            ( strpos($meta, '"widgetType":"trx_widget_slider"') !== false && strpos($meta, '"engine":"revo"') !== false )  // Our widget slider with param "engine" == "revo"
                            ||
