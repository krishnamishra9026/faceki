...
class trx_addons_widget_slider extends TRX_Addons_Widget {
	...
	function widget( $args, $instance ) {
		...
		// Widget body
		$html = '';
		if (in_array($engine, array('swiper'))) {
			...
			$noresize = isset($instance['noresize']) ? (int) $instance['noresize'] : 0;
                        $height = isset($instance['height']) && $noresize == 1 ? $instance['height'] : 0;
...
if ( !function_exists( 'trx_addons_sc_widget_slider' ) ) {
	function trx_addons_sc_widget_slider($atts, $content=null){	
		...
		if ((isset($atts['slides']) && !empty($atts['slides']) && count($atts['slides'])) == 0 ||
            (isset($atts['slides'][0]) && count($atts['slides'][0]) == 0) ||
            (empty($atts['slides'][0]['image'])
                && empty($atts['slides'][0]['video_url']) && empty($atts['slides'][0]['video_embed']))) {
				...
				array(		
					...
					array(
						...
						"value" => array(
							...
							esc_html__('Bottom Left', 'trx_addons') => 'bottom_left',
