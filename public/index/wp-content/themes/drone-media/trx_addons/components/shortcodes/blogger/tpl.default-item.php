...
?><div <?php post_class( 'sc_blogger_item post_format_'.esc_attr($post_format) ); ?>><?php
	...
	?><div class="sc_blogger_item_content entry-content"><?php
		...
		// Post title
		if ( !in_array($post_format, array('aside', 'status', 'quote')) ) {
		...
		if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) {
			?><div class="sc_blogger_item_excerpt">
				<div class="sc_blogger_item_excerpt_text">
					<?php
					$show_more = !in_array($post_format, array('aside', 'status', 'quote'));
					if (has_excerpt()) {
						the_excerpt();
				...
				// More button
				if ( $show_more ) {
					?><div class="sc_blogger_item_button sc_item_button"><a href="<?php echo esc_url($post_link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button', 'sc_blogger', $args)); ?>"><?php esc_html_e('Read more', 'drone-media'); ?></a></div><?php
				}
