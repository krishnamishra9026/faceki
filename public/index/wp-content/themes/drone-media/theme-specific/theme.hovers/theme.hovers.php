...
if (!function_exists('drone_media_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'drone_media_hovers_theme_setup3', 3 );
	function drone_media_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		drone_media_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'drone-media'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'drone-media') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'drone-media')
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'drone-media'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'drone-media') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'drone-media'),
					'icon'	=> esc_html__('Icon',	'drone-media'),
					'icons'	=> esc_html__('Icons',	'drone-media'),
					'zoom'	=> esc_html__('Zoom',	'drone-media'),
					'fade'	=> esc_html__('Fade',	'drone-media'),
					'slide'	=> esc_html__('Slide',	'drone-media'),
					'pull'	=> esc_html__('Pull',	'drone-media'),
					'border'=> esc_html__('Border',	'drone-media')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('drone_media_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'drone_media_hovers_theme_setup9', 9 );
	function drone_media_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'drone_media_hovers_frontend_scripts', 1010 );
		add_filter( 'drone_media_filter_localize_script','drone_media_hovers_localize_script' );
		add_filter( 'drone_media_filter_merge_scripts',	'drone_media_hovers_merge_scripts' );
		add_filter( 'drone_media_filter_merge_styles',	'drone_media_hovers_merge_styles' );
		add_filter( 'drone_media_filter_get_css', 		'drone_media_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'drone_media_hovers_frontend_scripts' ) ) {
	function drone_media_hovers_frontend_scripts() {
		if ( drone_media_is_on(drone_media_get_theme_option('debug_mode')) && drone_media_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'drone-media-hovers', drone_media_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( drone_media_is_on(drone_media_get_theme_option('debug_mode')) && drone_media_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'drone-media-hovers',  drone_media_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('drone_media_hovers_merge_scripts')) {
	function drone_media_hovers_merge_scripts($list) {
...
if (!function_exists('drone_media_hovers_merge_styles')) {
	function drone_media_hovers_merge_styles($list) {
...
if (!function_exists('drone_media_hovers_localize_script')) {
	function drone_media_hovers_localize_script($arr) {
		$arr['button_hover'] = drone_media_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('drone_media_hovers_add_icons') ) {
	function drone_media_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat'])) {
					drone_media_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? ($product->is_type( 'variable' ) ? esc_html__('Select options', 'drone-media') : esc_html__('Buy now', 'drone-media')) : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (!is_object($args['cat']))
							esc_html_e('View products', 'drone-media');
						else
							esc_html_e('Details', 'drone-media');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'drone_media_hovers_get_css' ) ) {
	function drone_media_hovers_get_css($css, $colors, $fonts, $scheme='') {
