...
if ( !function_exists('drone_media_need_page_title') ) {
	function drone_media_need_page_title() {
		return apply_filters('drone_media_filter_need_page_title', true);
	}
}
...
if ( !function_exists('drone_media_show_layout') ) {
	function drone_media_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('drone_media_get_logo_image') ) {
	function drone_media_get_logo_image($type='') {
		$logo_image = '';
		if (drone_media_is_on(drone_media_get_theme_option('logo_retina_enabled')) && drone_media_get_retina_multiplier(2) > 1)
			$logo_image = drone_media_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) {
			if (empty($type) && function_exists('the_custom_logo')) {
				...
				$logo_image = drone_media_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
			}
		}
		return drone_media_remove_protocol_from_url($logo_image, false);
	}
}

// Return header video (if set)
if ( !function_exists('drone_media_get_header_video') ) {
	function drone_media_get_header_video() {
		$video = '';
		if (apply_filters('drone_media_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (drone_media_check_theme_option('header_video')) {
				$video = drone_media_get_theme_option('header_video');
...
if ( !function_exists('drone_media_show_post_meta') ) {
	function drone_media_show_post_meta($args=array()) {

		if (is_single() && drone_media_is_off(drone_media_get_theme_option('show_post_meta'))) return ' ';	// Space is need!
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post date
					...
					$dt = apply_filters('drone_media_filter_get_post_date', drone_media_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post counters
					...
					if ( ($output = drone_media_get_post_counters($args['counters'])) != '')
						drone_media_show_layout($output);
					else {
						if (!is_singular() || have_comments() || comments_open()) {
							$post_comments = get_comments_number();
							?>
							<a href="<?php echo esc_url(get_comments_link()); ?>" class="post_meta_item post_counters_item post_counters_comments icon-01-comment"><?php
								?><span class="post_counters_number"><?php
									echo esc_html($post_comments);
								?></span><?php
								?><span class="post_counters_label"><?php
									echo (1==$post_comments ? esc_html__('Comment', 'drone-media') : esc_html__('Comments', 'drone-media'));
								?></span>
							</a>
							<?php
						}
				...
				// Socials share
					...
					drone_media_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'drone-media'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'drone-media' ), '<span class="post_meta_item post_edit">', '</span>' );
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('drone_media_show_post_featured') ) {
	function drone_media_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => drone_media_get_theme_option('image_hover'),	// Hover effect
		...
		$thumb_size = !empty($args['thumb_size']) 
						? $args['thumb_size'] 
						: drone_media_get_thumb_size(is_attachment() || is_single() ? 'full' : 'big');
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? drone_media_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = drone_media_get_post_image();
				if (!empty($image)) 
					$image = drone_media_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . drone_media_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured post_attachment<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>">
					...
					if (!$args['thumb_bg']) 
						echo wp_get_attachment_image( get_the_ID(), $thumb_size, false, 
													 drone_media_is_on(drone_media_get_theme_option('seo_snippets'))
				...
				?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$drone_media_attr = drone_media_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($drone_media_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($drone_media_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						drone_media_hovers_add_icons($args['hover']);
					}
					...
					if (($output=drone_media_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						drone_media_show_layout($output);
					...
					if (!empty($image)) {
						...
						drone_media_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = drone_media_get_post_video($post_content, false);
						if (empty($video))
							$video = drone_media_get_post_iframe($post_content, false);


						$post_content = drone_media_get_post_content();
						$video = drone_media_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = drone_media_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							...
							$post_content_parsed = drone_media_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = drone_media_array_get_first( $videos, false );
								}
							}
						}
					...
					// Put audio over the thumb
						...
						$audio = drone_media_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = drone_media_get_post_iframe($post_content, false);
						if (!empty($audio)) {
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
								// Add author and title
								$media_author = drone_media_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = drone_media_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !drone_media_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php drone_media_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !drone_media_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php drone_media_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								drone_media_show_layout($audio); 
							?></div><?php
						}
					}
				}
				...
				drone_media_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				drone_media_show_layout($post_info);
...
if ( !function_exists('drone_media_get_no_image') ) {
	function drone_media_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = drone_media_get_theme_option( 'no_image' );
			if (empty($img)) $img = drone_media_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('drone_media_add_bg_in_post_nav') ) {
	function drone_media_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = drone_media_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), drone_media_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), drone_media_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'drone-media-main', $css );
	}
}

// Show related posts
if ( !function_exists('drone_media_show_related_posts') ) {
	function drone_media_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
		...
		$taxonomy = $args['post_type'] == 'post' ? 'category' : drone_media_get_post_type_taxonomy();
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'drone-media');
				?></h3>
...
if ( !function_exists('drone_media_show_portfolio_posts') ) {
	function drone_media_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? drone_media_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = drone_media_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = drone_media_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the drone_media_ajax_get_posts action
if ( !function_exists( 'drone_media_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_drone_media_ajax_get_posts',			'drone_media_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_drone_media_ajax_get_posts',	'drone_media_ajax_get_posts_callback');
	function drone_media_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			drone_media_storage_set('blog_mode', 'blog');
			drone_media_storage_set('options_meta', get_post_meta($id, 'drone_media_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => drone_media_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'drone-media');
		}
...
// Show pagination
if ( !function_exists('drone_media_show_pagination') ) {
	function drone_media_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = drone_media_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'drone-media' ),
				'next_text' => esc_html__( '>', 'drone-media' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'drone-media' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'drone-media'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'drone-media') : esc_html__('Newest posts', 'drone-media') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'drone-media') : esc_html__('Older posts', 'drone-media'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'drone_media_single_comments_field' ) ) {
    function drone_media_single_comments_field( $args ) {
